...
// In-memory store — survives warm instances, resets on cold start.
// Good enough for the MVP (one active review at a time).
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (body.action === 'approve') {
      if (pendingReview) pendingReview.status = 'approved';
    ...
    if (body.action === 'reject') {
      if (pendingReview) pendingReview.status = 'rejected';
