...
function logRealExecutions(jobResults) {
  ...
  allWithName
    .filter(e => e.timestamp || e.createdAt)
    .sort((a, b) => new Date(b.timestamp || b.createdAt) - new Date(a.timestamp || a.createdAt))
    .slice(0, 5)
    .reverse() // oldest first so feed reads chronologically
    .forEach(e => {
      const level = e.status === 1 ? 'SUCCESS' : (e.status != null && e.status !== 1) ? 'ERROR' : 'INFO';
      const dur = formatDuration(e.duration || e.durationMs);
      const ops = e.operations ? ` | ${e.operations} ops` : '';
      const t = new Date(e.timestamp || e.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      ...
      scenarios.map(async scenario => {
        try {
          const execData = await makeApiGet(
            `scenarios/${scenario.id}/logs`,
            { limit: CONFIG.EXECUTIONS_PER_JOB }
          );
          const executions = execData.scenarioLogs || [];
